...
async function installHostSpecificPackages(ip) {
  ...
  command = `sudo ln -sf ~/.nvm/versions/node/vv20.12.0/bin/npm /usr/bin/npm &&
                    sudo ln -sf ~/.nvm/versions/node/vv20.12.0/bin/node /usr/bin/node &&
                    sudo ln -sf ~/.nvm/versions/node/vv20.12.0/bin/npx /usr/bin/npx`
  await runSshCommand(ip, command, maxRetries)
